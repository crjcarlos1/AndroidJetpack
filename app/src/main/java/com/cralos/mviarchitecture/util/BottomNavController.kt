...
class BottomNavController(
    ...
    lateinit var navItemChangeListener: OnNavigationItemChenge
    ...
    fun onBackPressed() {
        ...
        when {
            ...
            navigationBackStack.size > 1 -> {
                //remove last item for backstack
                navigationBackStack.removeLast()

                //update the container  with new fragment
                onNavigationItemSelected()
            }
            // if the stack has only one  and it's not the navigation  home  we should ensure
            //  that the aplication always leave from startDestination
    ...
    interface OnNavigationItemChenge {
        fun onItemChange(itemId: Int)
    }

    fun setOnItemNavigationChanged(listener: (itemId: Int) -> Unit) {
        this.navItemChangeListener = object : OnNavigationItemChenge {
